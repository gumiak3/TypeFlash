...
import { setState, setStatesForWholeWord } from '../../../store/wordsStates/wordsStatesSlice';
...
export class TypingHandler {
  ...
  private clearLetterState(wordId: number, letterId: number) {
    this.dispatch(setState({ wordId: wordId, letterId: letterId, newState: '' }));
  }
  ...
  private handleBackspace(): void {
    ...
    if (letterId === 0 && wordId - 1 >= 0) {
      ...
      const newCursorPosition = {
        top: this.letterProps[wordId - 1][this.dictionary[wordId - 1].length - 1].top,
        left: this.letterProps[wordId - 1][this.dictionary[wordId - 1].length - 1].right,
        getRightValue: true
      };
      ...
      this.cursor.position.left === this.letterProps[wordId][letterId].right
    ) {
      this.clearLetterState(wordId, letterId);
      const newCursorPosition = {
  ...
  private handleJumpToNextWord() {
    ...
    this.cursorHandler.updateCursor(wordId + 1, 0, newCursorPosition, this.cursor.position);
    ...
    if (this.isAlphabet(e.key) && currentLetter < currentWord.length) {
      ...
      this.dispatch(
        setState({
          ...
          newState: letters[currentLetter] === e.key ? 'correct' : 'incorrect'
        })
      );
